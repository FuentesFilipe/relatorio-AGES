...
Já na sprint 0 a squad do \textit{frontend} desenvolveu o protótipo das telas, tendo em vista que a cliente já havia fornecido um \textit{design} inicial desejado. 
Segue ilustrado o trabalho feito pela squad nas imagens 20 a 26.

\begin{figure}[H]
  \centering
  \small
  \caption{Protótipo das telas inicial, login, cadastro, edição de cadastro e recuperação de senha}
  \includegraphics[width=1\linewidth]{conteudo//3 - ages II//conteudo//figures/telas-1.png}
  Fonte: Autor
  \label{fig:telas-1}
\end{figure}
...
\begin{figure}[H]
  \centering
  \small
  \caption{Protótipo das telas de aulas gravadas e edição de cadastro}
  \includegraphics[width=1\linewidth]{conteudo//3 - ages II//conteudo//figures/telas-2.png}
  Fonte: Autor
  \label{fig:telas-2}
\end{figure}
...
\begin{figure}[H]
  \centering
  \small
  \caption{Protótipo das telas de devocionais e orações}
  \includegraphics[width=1\linewidth]{conteudo//3 - ages II//conteudo//figures/telas-3.png}
  Fonte: Autor
  \label{fig:telas-3}
\end{figure}
...
\begin{figure}[H]
  \centering
  \small
  \caption{Protótipo das telas de edição de cadastro e diário emocional}
  \includegraphics[width=1\linewidth]{conteudo//3 - ages II//conteudo//figures/telas-4.png}
  Fonte: Autor
  \label{fig:telas-4}
\end{figure}
...
\begin{figure}[H]
  \centering
  \small
  \caption{Protótipo da tela de home}
  \includegraphics[width=1\linewidth]{conteudo//3 - ages II//conteudo//figures/telas-5.png}
  Fonte: Autor
  \label{fig:telas-5}
\end{figure}
...
\begin{figure}[H]
  \centering
  \small
  \caption{Protótipo das telas de boas-vindas, perfil emocional e resultado do perfil emocional}
  \includegraphics[width=1\linewidth]{conteudo//3 - ages II//conteudo//figures/telas-6.png}
  Fonte: Autor
  \label{fig:telas-6}
\end{figure}
...
De uma maneira geral a tecnologia mais utilizada foi a linguagem TypeScript, tanto no \textit{frontend} quanto no \textit{backend} e diferenciando-se apenas nos \textit{frameworks} utilizados. No \textit{frontend} foi utilizado o React Native, enquanto no \textit{backend} foi utilizado o NestJS. Além disso, o \ac{sgbd} utilizado foi o Supabase, que é um banco de dados PostgreSQL com uma camada de abstração para facilitar o uso. A camada de persistência da API foi implementada com o Prisma ORM, incluindo a execução de migrações do esquema de dados \cite{typescript,react-native,nestjs,supabase,postgresql,prisma}.

Recursos da AWS também foram utilizados, como o EC2, S3 e API Gateway. O EC2 foi utilizado para hospedar a API, o S3 para armazenar arquivos e o API Gateway para redirecionar chamadas para a instância EC2. O Docker e o GitLab CI também integraram o fluxo de disponibilização da aplicação \cite{aws-docs,docker,gitlab-ci}.
